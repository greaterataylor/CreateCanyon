...
export const apiEnvironmentSchema = baseEnvironmentSchema
  ...
  .extend({
    ...
    TYPESENSE_URL: z.string().url().default("http://127.0.0.1:8108"),
    TYPESENSE_API_KEY: z.string().min(8).default("local-typesense-key"),
    TAX_MODE: z.enum(["disabled", "stripe"]).default("disabled"),
    STRIPE_TAX_CODE: z.string().default("txcd_10000000"),
    PRODUCTION_LAUNCH_APPROVED: booleanFromString.default(false),
    OIDC_JWKS_URL: z.string().url().optional(),
  })
  .superRefine((environment, context) => {
    if(Boolean(environment.S3_ACCESS_KEY_ID)!==Boolean(environment.S3_SECRET_ACCESS_KEY))context.addIssue({code:"custom",message:"Supply both S3 access credentials, or neither to use an IAM role"});
    if (environment.NODE_ENV === "production") {
      if (!environment.PRODUCTION_LAUNCH_APPROVED || environment.TAX_MODE !== "stripe") {
        context.addIssue({ code: "custom", message: "Production launch approval and Stripe Tax configuration are required" });
      }
      if (environment.CORS_ALLOWED_ORIGINS.some((origin) => !origin.startsWith("https://"))) {
        context.addIssue({ code: "custom", message: "Production application origins must use HTTPS" });
      }
...
export const webEnvironmentSchema = baseEnvironmentSchema.extend({
  ...
  OIDC_AUDIENCE: z.string().default("createcanyon-api"),
  OIDC_STEP_UP_ACR: z.string().optional(),
  SESSION_MAX_AGE_SECONDS: z.coerce.number().int().min(300).max(28800).default(28800),
});
