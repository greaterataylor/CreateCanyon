...
/** Database configuration is the authority for channel eligibility. */
export const channelEligibility = pgTable("channel_eligibility", {
  storefrontId: uuid("storefront_id").notNull().references(() => storefronts.id, { onDelete: "cascade" }),
  assetType: assetTypeEnum("asset_type").notNull(),
  enabled: boolean("enabled").notNull().default(true),
}, (t) => [primaryKey({ columns: [t.storefrontId, t.assetType] })]);

export const sellerTerms = pgTable("seller_terms", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const sellerTermsAcceptances = pgTable("seller_terms_acceptance", {
  id: uuid("id").primaryKey().defaultRandom(),
  sellerOrganisationId: uuid("seller_organisation_id").notNull().references(() => sellerOrganisations.id),
  termsId: uuid("terms_id").notNull().references(() => sellerTerms.id),
  userId: uuid("user_id").notNull().references(() => users.id),
  contentHash: varchar("content_hash", { length: 64 }).notNull(),
  acceptedAt: timestamp("accepted_at", { withTimezone: true }).notNull().defaultNow(),
}, t => [uniqueIndex("seller_terms_acceptance_unique").on(t.sellerOrganisationId, t.termsId)]);
export const supportMessages = pgTable("support_message", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, t => [index("support_message_case_idx").on(t.caseId, t.createdAt)]);
...
export const refundRequests = pgTable("refund_request", {
  ...
  amountMinor: bigint("amount_minor", { mode: "bigint" }).notNull(),
  sellerMinor: bigint("seller_minor", { mode: "bigint" }).notNull(),
  ...
  ...timestamps,
}, t => [index("refund_line_idx").on(t.orderLineId, t.status), check("refund_positive", sql`${t.amountMinor} > 0`)]);
export const transferReversals = pgTable("transfer_reversal", {
  id: uuid("id").primaryKey().defaultRandom(),
  transferId: uuid("transfer_id").notNull().references(() => transfers.id),
  refundId: uuid("refund_id").notNull().references(() => refundRequests.id).unique(),
  providerReversalId: text("provider_reversal_id").unique(),
  amountMinor: bigint("amount_minor", { mode: "bigint" }).notNull(),
  status: varchar("status", { length: 32 }).notNull().default("REQUESTED"),
  ...timestamps,
});
export const paymentDisputes = pgTable("payment_dispute", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  fundsWithdrawn: boolean("funds_withdrawn").notNull().default(false),
  fundsReinstated: boolean("funds_reinstated").notNull().default(false),
  metadata: jsonb("metadata").$type<Record<string, unknown>>().notNull().default({}),
  ...timestamps,
});
...
export const scanResults = pgTable("scan_result", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, t => [uniqueIndex("scan_file_scanner_unique").on(t.fileObjectId,t.scanner)]);
export const wishlists = pgTable("wishlist_item", {
  userId: uuid("user_id").notNull().references(() => users.id),
