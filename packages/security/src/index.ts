...
export function normalizeHostname(value: string): string {
  ...
  if(input.startsWith("[")){const match=input.match(/^\[([0-9a-f:]+)\](?::[0-9]{1,5})?$/);return match?`[${match[1]}]`:"";}
  if(!/^[a-z0-9.-]+(?::[0-9]{1,5})?$/.test(input))return "";
  return input.replace(/:[0-9]+$/,"").replace(/\.$/,"");
}
export function resolveStorefrontFromHost(hostHeader:string,allowlist:readonly string[]):StorefrontIdentity {
  const host=normalizeHostname(hostHeader);if(!host||!new Set(allowlist.map(normalizeHostname)).has(host))throw new HostResolutionError(host);
  const base=host.replace(/^www\./,"").replace(/\.localhost$/, ".com");
  const storefront=["localhost","127.0.0.1","[::1]"].includes(host)?storefrontIdentities.createcanyon:allStorefronts.find(s=>s.hostname===base);
  if(!storefront)throw new HostResolutionError(host);return storefront;
}
