...
export const checkoutSchema = z.object({
  ...
  billingAddress: z.object({ line1: z.string().min(1).max(200), line2: z.string().max(200).optional(), city: z.string().min(1).max(100), state: z.string().max(100).optional(), postal_code: z.string().min(1).max(32), country: z.string().regex(/^[A-Z]{2}$/) }).optional(),
