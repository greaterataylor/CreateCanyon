...
import { upsertLicenseTemplate } from '@/lib/settings'
...
export const POST = withRedirectAuth(async function POST(req: NextRequest) {
  ...
  const parsed = licenseTemplateSchema.safeParse({
    key: slugify(form.get('key')?.toString() || form.get('name')?.toString() || ''),
  ...
  await createAuditLog({ actorUserId: admin.id, siteId: site.id, entityType: 'siteLicenseTemplate', entityId: parsed.data.key, action: 'license-template.saved', details: { name: parsed.data.name, isDefault: parsed.data.isDefault } })
