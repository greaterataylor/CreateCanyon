            ...
            <div className="flex flex-wrap items-center gap-4">
              {site.logoUrl ? (
                <div className="flex-shrink-0">
                  <img src={site.logoUrl} alt={site.name} className="h-24 w-auto max-w-[220px] object-contain object-left sm:h-28 sm:max-w-[260px]" />
                </div>
              ) : null}
              <div>
                <h1 className={site.logoUrl ? 'sr-only' : 'text-3xl font-bold sm:text-4xl'}>{home.title}</h1>
                <p className={`${site.logoUrl ? '' : 'mt-2 '}max-w-2xl text-gray-600`}>{home.description}</p>
              </div>
            </div>
