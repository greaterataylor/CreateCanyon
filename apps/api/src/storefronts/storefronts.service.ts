...
export class StorefrontsService {
  ...
  public async get(key: StorefrontKey) {
    ...
    const eligibility = await this.connection.client`SELECT asset_type FROM channel_eligibility WHERE storefront_id=${storefront.id} AND enabled`;
    return { ...storefront, categories: categoryRows, eligibleAssetTypes: eligibility.map(r => r.asset_type) };
  }
}
