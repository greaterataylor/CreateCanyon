...
export function validateAssetAgainstCategoryRules(category: any, data: any, metadata?: Record<string, unknown>) {
  ...
  const structuredMetadata = metadata ?? ((data && typeof data.metadata === 'object' && !Array.isArray(data.metadata)) ? data.metadata : {})
  const metadataValidation = validateStructuredMetadata(fieldTemplates, structuredMetadata)
...
export function buildAssetMetadataEntries(category: any, metadata: Record<string, unknown>) {
  return buildMetadataEntries(getCategoryFieldTemplates(category), metadata)
}
